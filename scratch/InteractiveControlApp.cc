...
main (int argc, char *argv[])
{
  ...
  uint32_t  TMCInterval = 30;
  uint32_t  VideoBundleSize = 32000; // in bytes
  uint32_t  SerialDataBundleSize = 4800; // in bytes
  uint32_t  SimStart = 30;
  uint32_t  SimEnd = 60;
  string QueueLenpath;
  uint32_t  m_InterfaceNum;
  bool HandOver = true;
  uint32_t HandOverTime = 15;//seconds
  bool LoadBalance = false;
  uint32_t ForwardingQueue = 2147483648;
  string InteractiveSatelliteString = "3,7";
  ...
  cmd.AddValue ("VideoBundleSize", "size (in bytes) of a video stream bundle", VideoBundleSize);
  cmd.AddValue ("TMTCInterval", "path of channel selextion result", TMCInterval);
  cmd.AddValue ("SerialDataBundleSize", "size (in bytes) of a serial data bundle", SerialDataBundleSize);
  ...
  cmd.AddValue ("QueueSize", "Size (in packets) of the forwarding queues", ForwardingQueue);
  cmd.AddValue ("InteractiveSatellites", "Comma seperated list of the satellite IDs that runs an interactive control application", InteractiveSatelliteString);
  ...
  // parse satellite IDs from InteractiveSatelliteString
  vector<uint32_t> InteractiveSatellites;
  int prevDelim = 0;
  int delim = -1;
  while ((delim = InteractiveSatelliteString.find(',', delim + 1)) != string::npos) {
	  uint32_t satid = stoi(InteractiveSatelliteString.substr(prevDelim, delim));
	  InteractiveSatellites.push_back(satid);
	  prevDelim = delim + 1;
  }
  uint32_t satid = stoi(InteractiveSatelliteString.substr(prevDelim, delim));
  InteractiveSatellites.push_back(satid);
...
//2147483648
  ...
  for (uint16_t kk = 0; kk < Nnodes ; kk++)
	...
	for (uint32_t sid : InteractiveSatellites) {
	    SpiderClient * clientAppVideo = new SpiderClient;
	    clientAppVideo->SetRemote (Mac48Address::ConvertFrom (nodesGES.Get(0)->GetDevice(0)->GetAddress()) );
	    clientAppVideo->SetMaxPackets (2147483648);
	    clientAppVideo->SetInterval (Seconds(1)); //1
	    clientAppVideo->SetPacketSize (VideoBundleSize);
	    clientAppVideo->SetQos (2);
	    clientAppVideo->SetConstantRate (true);
	    clientAppVideo->SetOutputpath (OutputpathSender);
	    clientAppVideo->InstallDevice (nodes.Get(sid)->GetDevice(0));
	    clientAppVideo->StartApplication (Seconds (SimStart));
	    clientAppVideo->StopApplication (Seconds (SimEnd));
	
		// serial data downlink
	    SpiderClient * clientAppSerialDown = new SpiderClient;
	    clientAppSerialDown->SetRemote (Mac48Address::ConvertFrom (nodesGES.Get(0)->GetDevice(0)->GetAddress()) );
	    clientAppSerialDown->SetMaxPackets (2147483648);
	    clientAppSerialDown->SetInterval (Seconds(1)); //1
	    clientAppSerialDown->SetPacketSize (SerialDataBundleSize);
	    clientAppSerialDown->SetQos (2);
	    clientAppSerialDown->SetConstantRate (true);
	    clientAppSerialDown->SetOutputpath (OutputpathSender);
	    clientAppSerialDown->InstallDevice (nodes.Get(sid)->GetDevice(0));
	    clientAppSerialDown->StartApplication (Seconds (SimStart));
	    clientAppSerialDown->StopApplication (Seconds (SimEnd));
	
		// serial data downlink
	    SpiderClient * gesAppSerialUp = new SpiderClient;
	    gesAppSerialUp->SetRemote (Mac48Address::ConvertFrom (nodes.Get(sid)->GetDevice(0)->GetAddress()) );
	    gesAppSerialUp->SetMaxPackets (2147483648);
	    gesAppSerialUp->SetInterval (Seconds(1)); //1
	    gesAppSerialUp->SetPacketSize (SerialDataBundleSize);
	    gesAppSerialUp->SetQos (2);
	    gesAppSerialUp->SetConstantRate (true);
	    gesAppSerialUp->SetOutputpath (OutputpathSender);
	    gesAppSerialUp->InstallDevice (nodesGES.Get(0)->GetDevice(0));
	    gesAppSerialUp->StartApplication (Seconds (SimStart));
	    gesAppSerialUp->StopApplication (Seconds (SimEnd));
	}
