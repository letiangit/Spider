...
public:
   ...
   */
  virtual bool TransmitStart (Ptr<Packet> p, Ptr<PointToPointNetDevice> src, Time txTime, uint32_t linkchannel);
...
protected:
  ...
  uint32_t GetLinkChannel (uint32_t i) const;
...
public:
    ...
    uint32_t                   m_channel; //one satellite support 4 link channel
